    ...
    public List<Map> search(String value) throws Exception{
        ...
        try {
            ...
            for (int i = 0; i < hits.length; i++) {
                ...
                TokenStream tokenStream = analyzer.tokenStream("summary",new StringReader(name));
                //传入的第二个参数是查询的值
                TextFragment[] frag = highlighter.getBestTextFragments(tokenStream, name, false, 10);
                //String baikeValue="";
                StringBuffer baikeValue = new StringBuffer();
                for (int j = 0; j < frag.length; j++) {
                if ((frag[j] != null) && (frag[j].getScore() > 0)) {
//                    if ((frag[j] != null)) {
                        //获取 summary 的值
                        //baikeValue=baikeValue+((frag[j].toString()));
                        baikeValue.append(frag[j].toString());
                    }
                }
                ...
                //map.put("score",score);
                map.put("score",Float.toString(score));
                list.add(map);
            }
        } catch (IOException e) {
            e.printStackTrace();
